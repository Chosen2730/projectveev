...
export default function Checkout() {
    ...
    useEffect(() => {
        if (user) {
            setUserDetails({ name: user.name, email: user.email, phone: user.phoneNumber })
        }
    }, [user]);
    ...
    const handleSuccess = async () => {
        const data = { name: userDetails.name, email: userDetails.email, phone: userDetails.phone, totalAmount, orderItems: cartItems }
                        ...
                        return (
                            <div className="item">
                                ...
                                <img
                                    ...
                                    src={img ? img : "https://images.unsplash.com/photo-1526947425960-945c6e72858f?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=2850&q=80"}
                                    alt={item}
                                />
                    ...
                    <div className="checkout-form">
                        <div className="checkout-field" style={{ margin: '30px 0' }}>
                            ...
                            <input
                                ...
                                value={userDetails.name} id="name" style={{ border: '1px solid', padding: '5px 10px' }} />
                        </div>
                        <div className="checkout-field" style={{ margin: '20px 0' }}>
                            <label>Email: </label>
                            <input
                                onChange={(e) => setUserDetails({ ...userDetails, email: e.target.value })}
                                value={userDetails.email} id="email" style={{ border: '1px solid', padding: '5px 10px' }} />
                        </div>
                        <div className="checkout-field">
                            <label>Phone: </label>
                            <input
                                onChange={(e) => setUserDetails({ ...userDetails, phone: e.target.value })}
                                value={userDetails.phoneNumber} id="phone" style={{ border: '1px solid', padding: '5px 10px' }} />
                        </div>
                    </div>
